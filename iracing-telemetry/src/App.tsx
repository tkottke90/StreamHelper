  ...
  useSignalEffect(() => {
    ...
    untracked(() => {
      TelemetryService.getTelemetry(
        iRacingPath.getFilePath(selectedFile.value)
      ).then(async (telemetry) => {
        const [initializedTelemetry] =
          await TelemetryService.loadRecords(telemetry);
        const firstRecord = await TelemetryService.getRecord(
  ...
  return (
    <div className="w-full h-full m-0 grid grid-cols-[200px_1fr] grid-rows-[56px_1fr]">
      ...
      <main className="p-4 col-span-2 w-full h-full overflow-auto">
        {info.value && lastRecord.value && (
          <TelemetryDisplay telemetry={info} lastData={lastRecord} />
        )}
      </main>
...
function Header({ onSelect }: { onSelect: (value: string) => void }) {
  ...
  useSignalEffect(() => {
    ...
    untracked(() => {
      TelemetryService.getTelemetryFiles(pathConfig.value).then(
        (files) => (directoryOptions.value = files)
      );
    });
  });
    ...
    <header className="py-2 px-4 col-span-2 bg-matisse-900 shadow flex justify-between">
      ...
      <div className="flex gap-2">
        ...
        >
          ...
          {directoryOptions.value.map((file) => (
            <FileOption file={file} key={iRacingPath.getFilePath(file)} />
          ))}
        </select>
...
function enumerateRecord<
  Values,
  Input extends { [s: string]: Values } = { [s: string]: Values }
>(record: Input) {
  return Object.entries<Values>(record).sort(([keyA], [keyB]) =>
    keyA > keyB ? 1 : -1
  );
}

export async function incrementShownData(
  telemetry: Telemetry,
  data: Signal<Record<string, TelemetryData>>,
...
function TelemetryDisplay({
  ...
  telemetry: Signal<Telemetry | undefined>;
  lastData: Signal<Record<string, TelemetryData>>;
}) {
  const currentIndex = useSignal(0);
  const prevDisabled = useComputed(
    () => !telemetry.value || currentIndex.value <= 0
  );
  const nextDisabled = useComputed(
    () =>
      !telemetry.value ||
      currentIndex.value >= telemetry.value.sampler.sample_count
  );
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="card">
        ...
        <br />
        <pre>
          <code>{JSON.stringify(telemetry.value.metadata, null, 2)}</code>
        </pre>
      </div>
      ...
      <div className="card">
        ...
        <p>
          Record: {currentIndex} / {telemetry.value.sampler.sample_count}
        </p>
        <br />
        ...
        <div className="overflow-y-auto h-[200px]">
          {!lastData.value && (
            <p className="text-center">Click Next To Load Data</p>
          )}
          {lastData.value && (
            <table>
              <thead>
              ...
              <tbody>
                {enumerateRecord<TelemetryData>(lastData.value).map(
                  ([variableName, variableDetails]) => (
                    <tr key={variableName}>
                      <td>{variableName}</td>
                      <td>{variableDetails.data_type}</td>
                      <td>
                        {variableDetails.data} {variableDetails.unit}
                      </td>
                    </tr>
                  )
                )}
          ...
          <button
            ...
            onClick={async () => {
              ...
              loadData(
                ...
                lastData,
                currentIndex,
                telemetry.value.sampler.sample_count
              );
            }}
            className="btn-primary--raised"
          >
...
function TemplateEditor() {
  ...
  return (
    <div>
      <div className="flex gap-2">
        <button
          className={compoundClass('', {
            'btn-primary--raised': mode.value === TemplateEditorStates.Edit
          })}
        ...
        <button
          className={compoundClass('', {
            'btn-primary--raised': mode.value === TemplateEditorStates.Preview
          })}
      ...
      {mode.value === TemplateEditorStates.Edit && (
        <textarea className="w-full text-black" value={template.value} />
      )}
...
function VariableDefinitions({
  ...
  definitions: Array<TelemetryVariable>;
}) {
  return (
    <table>
      <thead>
